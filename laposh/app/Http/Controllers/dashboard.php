...
$graph4=array_values($data4Arr);
     ...
     return view('management.static.index')->with('bookings',$bookings)->with('ongoing',$ongoing)->with('clients',$clients)->with('users',$users)->with('rooms',$rooms)->with('categories',$categories)->with('closed',$closed)->with('cancelled',$cancelled)->with('booking_stats',$booking_stats)->with('ongoing_stats',$ongoing_stats)->with('cancelled_stats',$cancelled_stats)->with('closed_stats',$closed_stats)->with('clients_stats',$clients_stats)->with('graph',$graph)->with('graph2',$graph2)->with('graph3',$graph3)->with('graph4',$graph4);
    }
}
